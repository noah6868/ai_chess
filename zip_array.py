...
def zip_array(array, data=0.):  # 压缩成稀疏数组
    ...
    for i in range(len(array)):
        for j in range(len(array[0])):
            if array[i][j] != data:
                cnt += 1
    zip_res.append([len(array), len(array[0]), cnt])
    for i in range(len(array)):
        for j in range(len(array[0])):
            if array[i][j] != data:
...
def recovery_array(array, data=0.):  # 恢复数组
    ...
    print(type(recovery_res[int(array[1][0])][int(array[1][1])]), type(array[1][2]))
    for i in range(1, len(array)):
        recovery_res[int(array[1][0])][int(array[1][1])] = array[i][2]
