...
pygame.init()
pygame.mixer.init()
pygame.mixer.music.load('bgm/yinzi.wav')
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            mouse_x, mouse_y = event.pos # 获取当前x，y坐标
            if not first_button:
                for i in range(10):
                    for j in range(9):
                        if abs(80 * j + 30 - mouse_x) < 30 and abs(72 * i + 25 - mouse_y) < 30: #　选中当前格子
                            # flag = False
                            first_button = True
                            start_i_j = j, i
                            fire_rect.center = (start_i_j[0] * x_ratio + x_bais, start_i_j[1] * y_ratio + y_bais)
                            # print(start_i_j ,'start')
                            # check_str += str(start_i_j[0])
                            # check_str += str(start_i_j[1])
                ...
                for i in range(10):
                    for j in range(9):
                        if abs(80 * j + 30 - mouse_x) < 30 and abs(72 * i + 25 - mouse_y) < 30:
                            first_button = False
                            end_i_j = j, i
                            # check_str += str(end_i_j[0])
                            # check_str += str(end_i_j[1])
                            # print(end_i_j,'end\n')
                            # print("check_str:", check_str)
